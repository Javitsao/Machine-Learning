...
for t in range(100):
    ...
    for i in range(X_train.shape[1]):
        ...
        for theata in midpoints:
            for s in [-1, 1]:
                predict = s * np.where(X_sort[:, i] <= theata, 1, -1)
                err = np.sum(w_sort[predict != sorted_labels]) / np.sum(w)
                #print(err)
                if err < best_err:
                    best_feature = i
                    best_theata = theata
    ...
    predict = best_s * np.where(X_train[:, best_feature] <= best_theata, 1, -1)
    G += a * predict
...
# min_Ein = max(rec_Ein)
Eout_G = np.mean(np.sign(G) != y_test_binary)
print("Eout_G:", Eout_G)
